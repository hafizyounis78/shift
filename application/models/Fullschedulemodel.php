...
class Fullschedulemodel extends CI_Model
{
	...
	function delete_shift_template()
	{
		extract($_POST);
		$this->db->where('id',$shiftTempId);
		$this->db->delete('dusseldorf_v3_shift_templates');
...
//					$dep_filter = "AND b.dep_id=".$dept_id;
		...
		/* SELECT GROUP_CONCAT( CONCAT( b.first_name, ' ', b.last_name,' (', dep_name,')' )
								SEPARATOR ', ' )*/
		$myquery = "SELECT sft.Special_shift,sft.start_date, sft.start_time, sft.end_time, sft.end_date,sft.lunch_break, sft.location_id,sft.type, loc.map_name as name, loc.map_id,deplocation.dep_name,deplocation.color, (
			...
			$myquery = " SELECT  sft.Special_shift,sft.start_date, sft.start_time, sft.end_time, sft.end_date,sft.lunch_break, sft.location_id,sft.type, loc.map_name as name,loc.map_id,deplocation.dep_name,deplocation.color,CONCAT( b.first_name, ' ', b.last_name ) as emp_name
								  FROM    dusseldorf_v3_shifts sft, task_map_dep loc,dusseldorf_users b,departments as deplocation
